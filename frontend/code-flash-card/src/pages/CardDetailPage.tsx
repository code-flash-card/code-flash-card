...
export default function CardDetailPage() {
  ...
  const { data: cardList, error } = useFetch<CardFromServer[]>(API_URL.card);
  useEffect(() => {
    try {
      if (!cardId) return;
      cardApi.addView({ id: cardId }).catch((e) => {
        throw e;
      });
    } catch (e) {
      alert(e);
  ...
  const currentIndex = (cards: CardFromServer[], cardId: string) =>
    cards.findIndex((card) => card.cardId.toString() === cardId);

  const isFirst = (index: number) => index === 0;
  const isLast = (index: number, cards: unknown[]) =>
    cards.length - 1 === index;
  const card: Card = cardList
    .filter((card) => card.cardId.toString() === cardId)
    .find((card) => ({
      answer: card?.answer ?? "",
  ...
  const filteredCardListByHashName = cardList.filter(
    (card) => hashName === card.hashtags[0].name
  );
    ...
    <>
      <CardDetailPageWrapper>
        <Styled.PageHeader>
          <BackSpaceBtn />
          <progress
            value={filteredCardListByHashName.findIndex(
        ...
        <CardDetailContainer>
          ...
          <Styled.BtnContainer>
            <Styled.Button
              ...
              disabled={isFirst(
                currentIndex(filteredCardListByHashName, cardId ?? "")
              )}
            >
              이전 문제
            </Styled.Button>
            <Button onClick={onClickAfter}>
              {isLast(
                currentIndex(filteredCardListByHashName, cardId ?? ""),
                filteredCardListByHashName
              )
                ? "완료 하기"
                : "다음 문제"}
            </Button>
